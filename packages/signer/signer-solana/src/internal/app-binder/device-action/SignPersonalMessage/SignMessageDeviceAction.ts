...
} from "@internal/app-binder/task/SendSignDataTask";

export type MachineDependencies = {
  readonly signMessage: (arg0: {
    input: SignDataTaskArgs;
...
> {
  ...
  extractDependencies(internalApi: InternalApi): MachineDependencies {
    const signMessage = async (arg0: { input: SignDataTaskArgs }) => {
      const result = await new SignDataTask(internalApi, arg0.input).run();
